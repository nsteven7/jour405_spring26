...
Looking at the time series visualization above, identify at least two noticeable patterns or anomalies. What questions would you want to investigate further based on this initial view of the data? How might these visual patterns translate into potential story angles?
  ...
  Accidents dip slightly around the start/end of a year, then steadily rise. Accidents dropped sharply in early-mid 2020, then rose again. The highest and lowest days took place exactly one month apart in late 2021.
  The difference between the highest and lowest days is interesting, figuring out what happened between that month could make for an interesting story.
...
Based on the day-of-week analysis, which day(s) have the highest and lowest average accidents? Why might these patterns exist?
  ...
  Monday through Wednesday have the lowest average accidents. Friday and Saturday have the highest. More people go out on Friday/Saturday to places other than work/school compared to Mon-Wed. People may also engage in more reckless behavior during these days (like driving under the influence), which could contribute to the higher average.
...
Choose one of the top 10 unusual days and research what might have happened on that date. Look for weather events, holidays, major incidents, or other factors that could explain the unusual number of accidents. Briefly describe your findings and how you would incorporate this context into a story.
  ...
  A large storm passed through Maryland on October 29, 2021 and caused flooding across the state. This could be a feasible explanation to the unusually high number of accidents that day.
...
accidents <- accidents |>
  ...
  mutate(
    dow_mean = mean(total),
    dow_sd = sd(total),
    dow_z_score = (total - mean_accidents / sd_accidents
)) |>
  ungroup()
...
Compare the days that have extreme z-scores overall versus those with extreme day-of-week z-scores. How do these lists differ? What journalistic insights can you draw from comparing these two different approaches to measuring "unusual" days?
  ...
  Overall z-scores cap at -3.4 and 4.7 at either end, while day-of-week z-scores cap at -2.5 and 54.4 (this MIGHT be user error) at either end. 18 of the 20 most extreme positive z-scores (total) were on Fridays and weekends, as were the day-of-week z-scores. 16 of the 20 most extreme negative z-scores (total) were on weekdays, while day-of-week z-scores only have 2 days with negative z-scores (Jan. 11, 2021 and Jan. 3, 2025).
  Comparing z-scores within smaller pools seems to net more extreme results than one big pool.
...
Based on the monthly pattern analysis, which months have the highest and lowest average accidents? Develop a hypothesis about why these seasonal patterns exist.
  ...
  The month with the highest average accidents is October, the lowest is January.
  January doesn't have a lot of major holidays outside of the new year, and people may travel less because of it and the cold. Winter is also considerably less rainy than Fall or Spring which makes the roads safer, and in the past decades snow has become increasingly infrequent.
  October is a fairly rainy month which may cause more accidents, but the sun also gets lower around rush hour and can inhibit a driver's ability to see the road from around 4:30 PM to sunset. 
...
For the extreme outliers identified (z-scores beyond ±3), what do they have in common?
  ...
  All of the outliers except for one (which was unusually low) occurred on a Friday or on the weekend. Most of the outliers are also positive z-scores.
...
Based on all the analyses performed, craft your best story pitch, including a headline and lede. Be sure to include what kinds of sources/people you'd talk to for the story.
  ...
  An interesting story could be about unsafe driving at the end of the week and how to stay safe on the roads. Sources could include traffic safety officials or people who have been in or deal with road accidents. Looking further into the reasons behind a given set of accidents woud also be useful. This would probably be a softer feature story.
  
Keeping safe on the most dangerous days to drive

Road accidents are more likely to happen on Fridays and weekends, according to a dataset from Prince George's County that spans from 2017 to 2025.
