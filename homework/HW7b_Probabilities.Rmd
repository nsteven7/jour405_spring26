...
wh_paid <- wh_salaries |>
  filter(SALARY > 0)
wh_paid |> summarize(mean_salary= mean(SALARY), median_salary= median(SALARY), sd_salary= sd(SALARY))
...
wh_salaries |>
  ...
  geom_vline(aes(xintercept = mean(SALARY)), color = "purple" , linetype = "dashed" , linewidth = 1) +
  geom_vline(aes(xintercept = mean(SALARY) - sd(SALARY)), color = "pink", linetype = "dashed", size= 1) +
  geom_vline(aes(xintercept = mean(SALARY) + sd(SALARY)), color = "pink", linetype = "dashed", size= 1)
```

**Describe the shape of the distribution. Is it roughly normal? Skewed? Where do most salaries cluster?**

Most salaries cluster within 1 SD of the mean, and the highest peak is below 1 SD of the mean. Higher salaries are spread thin above the mean.
...
**How closely does the data follow the empirical rule? What does any deviation tell you about the distribution?**
  ...
  I think something is wrong with my RStudio because the actual_pct and actual_count probably aren't supposed to be zero.
