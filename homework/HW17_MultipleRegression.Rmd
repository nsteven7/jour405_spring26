...
Unemployment and racial composition have the strongest correlations to voter turnout. Income and bachelor's degrees are only significantly correlated with one another but not anything else (income has a slightly stronger correlation with turnout), and age has no strong correlation with anything.
On the scatterplots, the strongest linear relationships are unemployment, racial composition, and population, but they also contain clusters of data instead of a continuous line.
...
Population, bachelor's degrees, and unemployment are the most insignificant variables. The adjusted r-squared and p-value show that the model is usable and has a statistically significant output.
...
Model 2 uses income, median age and racial composition as its independent variables and is marginally better than the full model. Model 3 is the least significant and uses population, bachelor's degrees and unemployment. Model 4 is slightly worse than Model 2 and the full model.
...
# Model 3
model3 <- lm(turnout ~ population + pct_bachelors_higher + 
                   pct_unemployed, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ income + pct_bachelors_higher + 
                   pct_unemployed + pct_white, data = md_turnout)
summary(model4)
