...
```{r}
model <- lm(Wins ~ Giveaways+ Takeaways + `Points Scored`, data = teams)
summary(model)
```
...
This model is somewhat useful because it can predict wins, but a quarter of those predictions will likely be incorrect. The model is significant but may not be the most practical means of predicting game outcomes.
...
```{r}
selected_vars <- teams |>  select(Giveaways, Takeaways, `Points Scored`, Wins)
cor(selected_vars)
```
...
Points Scored has the most impact on wins while Yards Gained has the least. When Yards Gained is removed from the equation, there is little change except for Takeaways becoming statistically significant. Replacing Yards Gained with Giveaways shows that they have a significant negative relationship with wins and weak negative relationships with Takeaways and Points Scored. Changing the factors around shows that generally, Yards Gained has little impact on wins regardless of other factors.
...
This model could be used to predict the outcome of a game that could be compared to the actual results. It would be at its most interesting if the game was either extremely close to or very far from the prediction.
