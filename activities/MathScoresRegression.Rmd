...
```{r}
schoolsPA <- read_csv("https://raw.githubusercontent.com/dwillis/jour405_files/refs/heads/main/montgomery_pa.csv")
```
...
```{r}
model <- lm(`% Met or Exceeded Math Expectations` ~ `FARMS Pct`, data = schoolsPA)
...
```{r}
schoolsPA$predicted <- predict(model)
